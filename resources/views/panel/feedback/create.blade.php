...
<div class="card card-gold">
    <div style="font-weight:700;color:#fff;">{{ $event->title }}</div>
    <div style="font-size:0.8rem;color:var(--text-dim);margin-top:4px;">{{ \Morilog\Jalali\Jalalian::fromDateTime($event->starts_at)->format('Y/m/d') }}</div>
</div>
...
<form method="POST" action="{{ route('panel.feedback.store', $event) }}">
    ...
    <div class="card">
        <div style="text-align:center;margin-bottom:1.5rem;">
            <div style="font-size:0.9rem;color:var(--text-dim);margin-bottom:1rem;">تجربه شما از این دورهمی چطور بود؟</div>
            ...
            <div id="stars" style="display:flex;justify-content:center;gap:0.5rem;direction:ltr;">
                ...
                <button type="button" class="star-btn" data-value="{{ $i }}"
                    ...
                    <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="var(--gold-2)" stroke-width="1.5" class="star-icon">
            ...
            <div id="rating-label" style="font-size:0.85rem;color:var(--gold-1);margin-top:0.75rem;min-height:1.2rem;"></div>
        </div>

        <div class="field">
            <label>نظر شما (اختیاری)</label>
            <textarea name="comment" rows="4" placeholder="تجربه‌تان را بنویسید..."
                style="width:100%;background:#0d0d0f;border:1px solid var(--border);border-radius:13px;padding:0.85rem 1rem;color:var(--text);font-family:inherit;resize:vertical;">{{ old('comment') }}</textarea>
        </div>

        <button type="submit" class="btn btn-gold">ثبت بازخورد</button>
    </div>
</form>
...
<script>
    ...
    function paint(val) {
        stars.forEach(s => {
            ...
            if (v <= val) { icon.setAttribute('fill', 'var(--gold-1)'); icon.setAttribute('stroke', 'var(--gold-1)'); }
            else { icon.setAttribute('fill', 'none'); icon.setAttribute('stroke', 'var(--gold-2)'); }
        });
    }
    stars.forEach(s => {
