...
@section('title', 'پادکست ' . ($show['title'] ?? $defaultTitle))
...
<div style="display:flex;align-items:flex-start;gap:0.75rem;margin-bottom:1.3rem;">
    <a href="{{ route('panel.podcast') }}" class="icon-btn" style="flex-shrink:0;margin-top:2px;">
    ...
    <div style="font-size:1.4rem;font-weight:800;letter-spacing:-0.5px;">{{ $show['title'] ?? $defaultTitle }}</div>
</div>
...
<div class="pod-hero">
    ...
    <div style="position:relative;display:flex;align-items:center;gap:1.1rem;">
        @if($show && !empty($show['image']))
            <img src="{{ $show['image'] }}" alt="{{ $show['title'] ?? $defaultTitle }}" class="pod-cover">
        @else
        ...
        <div style="flex:1;min-width:0;color:#fff;">
            <div style="font-size:1.3rem;font-weight:800;line-height:1.2;">{{ $show['title'] ?? $defaultTitle }}</div>
