...
@php
    // فیلمِ هفتهٔ جاری از سرویسِ متمرکز (با fallbackِ خودکارِ تنبل، هم‌راستا با صفحهٔ فیلم هفته).
    $todayFilm = optional(app(\App\Services\WeeklyMovie\WeeklyMovieAssigner::class)->currentWeekAssignment())->film;
@endphp
...
<div class="pk-mood @if($currentMood) is-done-avail @endif" id="pk-mood-card" data-done="{{ $currentMood ? '1' : '0' }}">
    ...
    <div class="pk-mood__thanks @if(!$currentMood) is-hidden @endif" id="pk-mood-thanks">
        ...
        <p class="pk-mood__thanks-text">ما در یک روایتیم، باهم حال بهتر رو تجربه میکنیم.</p>
